...
def microseconds_to_kilowhetstone_for_ravenscar_runtime (microseconds, task_criticality_level):
  multiplier = 0
  divider = 0
  # According to some evaluations:
  ...
  # print ("-------")
  # print (task_criticality_level + "-crit task")
  # print ("KW:", result)

  task_overhead = 0
  
  if task_criticality_level == 'LO':
    # overhead due to a LO-crit task:
    # https://gitlab.com/thesisBottaroMattia/ada-ravenscar-runtime-for-zynq7000-dual-core-supporting-mixed-criticality-systems/uploads/18d8b1cba57a7221640a263861d25ffe/image.png
    task_overhead = 1000 * (overhead_parameter.overhead_values[config.PLATFORM]['BI'] + overhead_parameter.overhead_values[config.PLATFORM]['AH'] + overhead_parameter.overhead_values[config.PLATFORM]['CSW'] + overhead_parameter.overhead_values[config.PLATFORM]['DU'] + overhead_parameter.overhead_values[config.PLATFORM]['CSW'])
  elif task_criticality_level == 'HI':
    # overhead due to a HI-crit task
    # https://gitlab.com/thesisBottaroMattia/ada-ravenscar-runtime-for-zynq7000-dual-core-supporting-mixed-criticality-systems/uploads/34dc6c3ffa8b2135fd0a5fd9c3b80a45/image.png
    task_overhead = 1000 * (overhead_parameter.overhead_values[config.PLATFORM]['BI'] + overhead_parameter.overhead_values[config.PLATFORM]['AH'] + overhead_parameter.overhead_values[config.PLATFORM]['CSW'] + overhead_parameter.overhead_values[config.PLATFORM]['AH'] + overhead_parameter.overhead_values[config.PLATFORM]['EEV'] + overhead_parameter.overhead_values[config.PLATFORM]['DU'] + overhead_parameter.overhead_values[config.PLATFORM]['CSW'])

  overall_execution_time = microseconds + task_overhead
  result = int ((result * microseconds) / overall_execution_time)
  
  # print ("Shrinked KW:", result)
  # print ("Execution time:", microseconds)
  # print ("Task overhead", task_overhead)
...
def clean_XML_and_Ada_Files(experiment_id):
  ...
  for dirname in os.listdir(config.MAST_analysis_path[experiment_id]):
    dirpath = os.path.join(config.MAST_analysis_path[experiment_id], dirname)
    if os.path.exists(dirpath) and dirname != '.gitkeep':
      rmtree(dirpath)
...
def save_taskset_as_Ada_On_RTEMS_On_XM (schema, experiment_id):
  if schema == 1:
    TSP_one_core_one_partition (experiment_id)
  elif schema == 2:
    TSP_partition_for_each_criticality_level (experiment_id)
  else:
    raise Exception ("Schema " + str(schema) + " is not supported.")

# It generates a partition for each criticality level. In the authors' model, two level of criticality are considered (HI, LO).
# Therefore, there are two partitions for each core.
# Each partition concerns an Ada application on top of RTEMS (XM -> RTEMS -> Ada app)
def TSP_partition_for_each_criticality_level (experiment_id):
  criticality_levels = ['HIGH', 'LOW']
  cores = ['c1', 'c2']
  common_folder = config.Ada_RTEMS_Common_Folder

  for approach in config.Ada_RTEMS_XM_Paths[experiment_id]:
    ...
    for taskset in root.findall('taskset'):
      periods = {'c1': {}, 'c2': {}}
      hyperperiods = {'c1': {}, 'c2': {}}
      slices_duration = {'c1': {}, 'c2': {}}
      taskset_id = int(taskset.find('tasksetid').text)

      for core in cores:
        for cl in criticality_levels:
          periods[core][cl] = []
          hyperperiods[core][cl] = 0
          slices_duration[core][cl] = 0

          if taskset_id != -1:
            if core == 'c1':
              if cl == 'LOW':
                slices_duration[core][cl] = str(taskset.find('timesliceLOpartitionc1').text)
              else:
                slices_duration[core][cl] = str(taskset.find('timesliceHIpartitionc1').text)
            else:
              if cl == 'LOW':
                slices_duration[core][cl] = str(taskset.find('timesliceLOpartitionc2').text)
              else:
                slices_duration[core][cl] = str(taskset.find('timesliceHIpartitionc2').text)
      
      if taskset_id != -1:

        taskset_name = taskset.find('executionid').text
        # dir containing partitions
        ...
        for core in cores:
          ...
          for cl in criticality_levels:
            ...
            Ada_Units[core][cl] = ''
            Ada_SOP_Specs[core][cl] = ''
            Ada_SOP_Body[core][cl] = ''
            Single_Execution_Data[core][cl] = ''
            Main_Units[core][cl] = ''
            Mains_Files_Name[core][cl] = ''

            Ada_Units_Files_Name[core][cl] = 'taskset_' + taskset_name + '_' + core + cl + '.ads'
            Tasksets_Name[core][cl] = str(taskset.find('executionid').text) + '_' + core + cl

            Srcs_Dir[core][cl] = taskset_dir + core + cl + '/'
            Apps_Dir[core][cl] = Srcs_Dir[core][cl] + 'app/'
            #print (Apps_Dir)

            os.mkdir(Srcs_Dir[core][cl])
            os.mkdir(Apps_Dir[core][cl])

            # Mains generation, one for each partition 
            Main_Units[core][cl] = 'main_' + Tasksets_Name[core][cl]
            Mains_Files_Name[core][cl] = Main_Units[core][cl] + '.adb'
            main = 'with System;\nwith Periodic_Tasks;\n\n'
            main += 'procedure ' + Main_Units[core][cl] + ' is\n'
            main += "\tpragma Priority (114);\n"
            main += 'begin\n\tPeriodic_Tasks.Init;\nend ' + Main_Units[core][cl] + ';'

            # create main unit Ada file
            f = open(Apps_Dir[core][cl] + Mains_Files_Name[core][cl], 'w')
            f.write(main)
            f.close()

            Ada_Units[core][cl] = 'with Ada.Real_Time; use Ada.Real_Time;\n\nwith Ada.Text_IO;\n\nwith System.Multiprocessors;\nuse System.Multiprocessors;\n\n'
            Ada_Units[core][cl] += 'with Single_Execution_Data;\nwith Production_Workload;\nwith Set_Of_Procedures;\nwith Workload_Manager;\nwith Initial_Delay;\n\nwith RTEMS;\nwith RTEMS.TASKS;\nwith RTEMS.CPU_Usage;\n\n'
            # Ada_Units[core][cl] += 'use Periodic_Tasks;\n\npackage taskset_' + Tasksets_Name[core][cl] + ' is\n\n'
            Ada_Units[core][cl] += 'package body Periodic_Tasks is\n\n'
            Ada_Units[core][cl] += '\tfunction Get_Longest_Hyperperiod return Natural is\n\t\tMax_H : Natural := Single_Execution_Data.Experiment_Hyperperiods(1);\n\tbegin\n'
            Ada_Units[core][cl] += '\t\tfor I in Single_Execution_Data.Experiment_Hyperperiods\'Range loop\n'
            Ada_Units[core][cl] += '\t\t\tif Single_Execution_Data.Experiment_Hyperperiods(I) > Max_H then\n\t\t\t\tMax_H := Single_Execution_Data.Experiment_Hyperperiods(I);\n\t\t\tend if;\n\t\tend loop;\n\n'
            Ada_Units[core][cl] += '\t\treturn Max_H;\n\t end Get_Longest_Hyperperiod;\n\n'
            Ada_Units[core][cl] += '\t------------\n\t--  Init  --\n\t------------\n\n'
            Ada_Units[core][cl] += '\tprocedure Init is\n\t\tNext_Period : constant Ada.Real_Time.Time := Ada.Real_Time.Time_First + Ada.Real_Time.Microseconds (Initial_Delay.Delay_Time);\n\t\tPeriod_To_Add : constant Ada.Real_Time.Time_Span := Ada.Real_Time.Microseconds (Get_Longest_Hyperperiod);\n\n'

            Ada_SOP_Specs[core][cl] += 'with RTEMS.TASKS;\n\npackage Set_Of_Procedures is\n\n'
            Ada_SOP_Body[core][cl] += 'with Ada.Real_Time; use Ada.Real_Time;\n\nwith Single_Execution_Data;\nwith Production_Workload;\nwith Workload_Manager;\nwith Initial_Delay;\n\nwith Ada.Text_IO;\n\n'
            Ada_SOP_Body[core][cl] += 'package body Set_Of_Procedures is\n\n\tprocedure Work (Period : Duration; ID : Natural);\n\tpragma No_Return (Work);\n\n'
            Ada_SOP_Body[core][cl] += '\tprocedure Work (Period : Duration; ID : Natural) is\n'
            Ada_SOP_Body[core][cl] += '\t\t--  Next_Period : Ada.Real_Time.Time := Ada.Real_Time.Time_First + Ada.Real_Time.Microseconds (Initial_Delay.Delay_Time);\n\t\t--  Period_To_Add : constant Ada.Real_Time.Time_Span := Ada.Real_Time.Microseconds (Period);\n\t\tI : Natural := 0;\n'
            Ada_SOP_Body[core][cl] += '\t\tFake_Status : RTEMS.STATUS_CODES;\n\t\tFake_Previous_Mode : RTEMS.MODE;\t\tDelayed_At : Ada.Real_Time.Time;\n\t\tWaked_At : Ada.Real_Time.Time;\n\t\tDeadline : Ada.Real_Time.Time;\n\t\tTime_To_Sleep : Duration := 0.0;\n\t\tNow : Ada.Real_Time.Time;\n'
            # Ada_SOP_Body[core][cl] += '\tbegin\n\t\tloop\n\t\t\tdelay until Next_Period;\n\n\t\t\tProduction_Workload.Small_Whetstone (Workload_Manager.Get_Workload (ID, I));\n\n\t\t\tI := I + 1;\n\n\t\t\tNext_Period := Next_Period + Period_To_Add;\n\t\tend loop;\n\tend Work;\n\n'
            ...
            current_partition_XML = taskset.find ('core1') if core == 'c1' else taskset.find ('core2')
            tasks_on_current_partition_XML = current_partition_XML.find('tasks')

          for t in tasks_on_current_partition_XML.findall('task'):
            criticality_level_current_task = str(t.find ('criticality').text)
            
            Ada_Units[core][criticality_level_current_task] += '\t\tID_T_' + str(t.find('ID').text) + ' : RTEMS.ID;\n'
            Ada_Units[core][criticality_level_current_task] += '\t\tSTATUS_T_' + str(t.find('ID').text) + ' : RTEMS.STATUS_CODES;\n'

            Ada_SOP_Specs[core][criticality_level_current_task] += '\tprocedure T_' + str(t.find('ID').text) + '_Body (ARGUMENT : in RTEMS.TASKS.ARGUMENT);\n'
            Ada_SOP_Specs[core][criticality_level_current_task] += '\tpragma Convention (C, T_' + str(t.find('ID').text) + '_Body);\n\n'

            Ada_SOP_Body[core][criticality_level_current_task] += '\tprocedure T_' + str(t.find('ID').text) + '_Body (ARGUMENT : in RTEMS.TASKS.ARGUMENT) is\n\t\tpragma Unreferenced (ARGUMENT);\n\t\tPeriod : Duration := ' + str((to_microseconds_for_Ada (t.find('period').text)) / 1000000) + ';\n'
            Ada_SOP_Body[core][criticality_level_current_task] += '\t\tID : Natural := ' + str(t.find('ID').text) + ';\n'
            Ada_SOP_Body[core][criticality_level_current_task] += '\tbegin\n\t\tWork (Period, ID);\n\tend T_' + str(t.find('ID').text) + '_Body;\n\n'

            periods[core][criticality_level_current_task].append (int(to_microseconds_for_Ada (t.find('period').text)))
          
          for cl in criticality_levels:
            Ada_Units[core][cl] += '\n\t\tID_T_LAST : RTEMS.ID;\n\t\tSTATUS_T_LAST : RTEMS.STATUS_CODES;\n\tbegin\n\n'
            Ada_SOP_Specs[core][cl] += '\tprocedure LAST (ARGUMENT : in RTEMS.TASKS.ARGUMENT);\n'
            Ada_SOP_Specs[core][cl] += '\tpragma Convention (C, LAST);\n\nend Set_Of_Procedures;'
            # Ada_SOP_Body[core][cl] += '\tprocedure LAST (ARGUMENT : in RTEMS.TASKS.ARGUMENT) is\n\t\tpragma Unreferenced (ARGUMENT);\n\t\tNext_Period : Ada.Real_Time.Time := Ada.Real_Time.Time_First + Ada.Real_Time.Nanoseconds (1_000_000_000);\n'
            Ada_SOP_Body[core][cl] += '\tprocedure LAST (ARGUMENT : in RTEMS.TASKS.ARGUMENT) is\n\t\tpragma Unreferenced (ARGUMENT);\n'
          ...
          for t in tasks_on_current_partition_XML.findall('task'):
            ...
            Ada_Units[core][criticality_level_current_task] += '\t\tRTEMS.TASKS.CREATE(\n'
            Ada_Units[core][criticality_level_current_task] += '\t\t\tRTEMS.BUILD_NAME(\'T\', \'_\', \'' + t_id_RTEMS_classic_api_format[0] + '\', \'' + t_id_RTEMS_classic_api_format[1] + '\'),\n'
            Ada_Units[core][criticality_level_current_task] += '\t\t\t' + str(144 - int(t.find('priority').text)) + ',\n\t\t\tRTEMS.MINIMUM_STACK_SIZE,\n\t\t\tRTEMS.PREEMPT,\n\t\t\tRTEMS.DEFAULT_ATTRIBUTES,\n\t\t\tID_T_' + str(t_id) + ',\n\t\t\tSTATUS_T_' + str(t_id) + '\n\t\t);\n\n'
          ...
          for t in tasks_on_current_partition_XML.findall('task'):
            criticality_level_current_task = str(t.find ('criticality').text)
            t_id = int(str(t.find('ID').text))
            Ada_Units[core][criticality_level_current_task] += '\t\tRTEMS.TASKS.START(\n'
            Ada_Units[core][criticality_level_current_task] += '\t\t\tID_T_' + str(t_id) + ',\n\t\t\tSet_Of_Procedures.T_' + str(t_id) + '_Body\'ACCESS,\n\t\t\t0,\n\t\t\tSTATUS_T_' + str(t_id) + '\n\t\t);\n\n'

          for cl in criticality_levels:
            Ada_Units[core][cl] += '\t\tdelay To_Duration (Ada.Real_Time.Microseconds (Get_Longest_Hyperperiod));\n\n'
            Ada_Units[core][cl] += '\t\t--  RTEMS.CPU_Usage.RESET;\n\n'
            
            if config.TSP_SCHEMA == 1:
              Ada_Units[core][cl] += '\t\tif Single_Execution_Data.Partition_Id = "P2" then\n\t\t\tdelay 1.5;\n\t\t\tAda.Text_IO.Put_Line ("Second Partition");\n\t\tend if;\n'
            ...
            elif config.TSP_SCHEMA == 2:
              Ada_Units[core][cl] += '\n\t\tAda.Text_IO.Put_Line ("");\n\n'
              Ada_Units[core][cl] += '\t\tif Single_Execution_Data.Partition_Id = "C1LOW" then\n\t\t\tAda.Text_IO.Put_Line ("New Execution");\n\t\t\tAda.Text_IO.Put_Line (Single_Execution_Data.Id_Execution);\n\t\t\tAda.Text_IO.Put_Line ("Taskset_Size: " & Single_Execution_Data.Taskset_Size);\n\t\t\tAda.Text_IO.Put_Line ("Criticality Factor: " & Single_Execution_Data.Criticality_Factor);\n\t\t\tAda.Text_IO.Put_Line ("HI_Crit_Proportion: " & Single_Execution_Data.HI_Crit_Proportion);\n\t\t\tAda.Text_IO.Put_Line ("Taskset_Utilization: " & Single_Execution_Data.Taskset_Utilization);\n\t\tend if;\n'
              Ada_Units[core][cl] += '\t\tif Single_Execution_Data.Partition_Id = "C1HIGH" then\n\t\t\tdelay 1.5;\n\t\tend if;\n'
              Ada_Units[core][cl] += '\t\tif Single_Execution_Data.Partition_Id = "C2LOW" then\n\t\t\tdelay 3.0;\n\t\tend if;\n'
              Ada_Units[core][cl] += '\t\tif Single_Execution_Data.Partition_Id = "C2HIGH" then\n\t\t\tdelay 4.5;\n\t\tend if;\n'
            ...
            f = open(Apps_Dir[core][cl] + 'periodic_tasks.adb', 'w')
            f.write(Ada_Units[core][cl])
            f.close()

            f = open(Apps_Dir[core][cl] + 'set_of_procedures.ads', 'w')
            f.write(Ada_SOP_Specs[core][cl])
            f.close()

            f = open(Apps_Dir[core][cl] + 'set_of_procedures.adb', 'w')
            f.write(Ada_SOP_Body[core][cl])
            f.close()
    
            copyfile (common_folder + 'initial_delay.ads', Apps_Dir[core][cl] + 'initial_delay.ads')
            copyfile (common_folder + 'production_workload.ads', Apps_Dir[core][cl] + 'production_workload.ads')
            copyfile (common_folder + 'production_workload.adb', Apps_Dir[core][cl] + 'production_workload.adb')
            copyfile (common_folder + 'production_workload.ads', Apps_Dir[core][cl] + 'production_workload.ads')
            copyfile (common_folder + 'periodic_tasks.ads', Apps_Dir[core][cl] + 'periodic_tasks.ads')
            copyfile (common_folder + 'workload_manager.ads', Apps_Dir[core][cl] + 'workload_manager.ads')
            copyfile (common_folder + 'config.h', Apps_Dir[core][cl] + 'config.h')
            copyfile (common_folder + 'Makefile_partition_' + core + cl, Apps_Dir[core][cl] + 'Makefile')
            copyfile (common_folder + 'Makefile_partition.am', Apps_Dir[core][cl] + 'Makefile.am')
            copyfile (common_folder + 'Makefile_partition.in', Apps_Dir[core][cl] + 'Makefile.in')
            copyfile (common_folder + 'rtems_init.c', Srcs_Dir[core][cl] + 'rtems_init.c')

            edit_partition_makefile (Apps_Dir[core][cl] + 'Makefile', 'PROGRAM=' + Main_Units[core][cl])
        ...
        for core in Single_Execution_Data:
          for cl in Single_Execution_Data[core]:
            hyperperiods[core][cl] = int(compute_hyperperiod(periods[core][cl]))
            hyperperiods[core][cl] = int(compute_hyperperiod(periods[core][cl]))

        for core in Single_Execution_Data:
          for cl in Single_Execution_Data[core]:
            Single_Execution_Data[core][cl] = 'with System.Multiprocessors;\nuse System.Multiprocessors;\n\n' 
            Single_Execution_Data[core][cl] += 'package Single_Execution_Data is\n\tpragma Preelaborate;\n\n\tNumb_Of_Partitions : Positive := 4;\n\n'
            Single_Execution_Data[core][cl] += '\tExperiment_Hyperperiods : array (1 .. Numb_Of_Partitions) of Natural := (1 => ' + str(hyperperiods['c1']['LOW']) + ', 2 => ' + str(hyperperiods['c1']['HIGH']) + ', 3 => ' + str(hyperperiods['c2']['LOW']) + ', 4 => ' + str(hyperperiods['c2']['HIGH']) + ');\n\n'

            Single_Execution_Data[core][cl] += '\tId_Experiment : Integer := ' + str(experiment_id) + ';\n\tApproach : String := "' + approach.upper() + '";\n\tTaskset_Id : Integer := ' + str(taskset_id) + ';\n\tPartition_Id : String := "' + (core+cl).upper() + '";\n\n'
            Single_Execution_Data[core][cl] += '\tId_Execution : String := "' + taskset_name + '";\n\n'

            Single_Execution_Data[core][cl] += '\t--  Needed to plot diagrams. These data are stored as strings in order to avoid issue related\n'
            Single_Execution_Data[core][cl] += '\t--  to differents types representations in differents languages (Python and Ada).\n'
            taskset_size = str(taskset.find('tasksetsize').text)
            taskset_utilization = format (float (str(taskset.find('tasksetutilization').text)), '.3f')
            criticality_factor = str(taskset.find('criticalityfactor').text)
            hi_crit_proportion = str(taskset.find('perc').text)
            
            Single_Execution_Data[core][cl] += '\tTaskset_Size : String := "' + taskset_size + '";\n\tTaskset_Utilization : String := "' + taskset_utilization + '";\n\tCriticality_Factor : String := "' + criticality_factor + '";\n\tHI_Crit_Proportion : String := "' + hi_crit_proportion + '";\n\n'
            Single_Execution_Data[core][cl] += 'end Single_Execution_Data;'         

            f = open(Apps_Dir[core][cl] + 'single_execution_data.ads', 'w')
            f.write(Single_Execution_Data[core][cl])
            f.close()   
          ...
          for task_XML in tasks_XML.findall('task'):
            ...
            number_of_JR = int(hyperperiods[core][criticality_level_current_task] // to_microseconds_for_Ada (task_XML.find('period').text))
          ...
          for cl in criticality_levels:
            f = open(Apps_Dir[core][cl] + 'workload_manager.adb', 'w')
            f.write(workload_manager_unit)
            f.close()

        #print (Ada_Units_Files_Name)
        #print (Srcs_Dir)
        #print (Apps_Dir)
        #print (Ada_SOP_Body)
        ...
        copyfile (common_folder + 'makefile_schema_2', taskset_dir + 'makefile')
        copyfile (common_folder + 'rules.mk', taskset_dir + 'rules.mk')
        copyfile (common_folder + 'xm_cf_schema_2.arm.xml', taskset_dir + 'xm_cf.arm.xml')

        tree_xm_cf = ET.parse(taskset_dir + 'xm_cf.arm.xml')
        ...
        for processor_xml in processor_table_xml.findall('Processor'):
          ...
          if processor_xml.attrib['id'] == str(0): # core 1
            plan_xml.attrib['majorFrame'] = str(int(slices_duration['c1']['LOW']) + int(slices_duration['c1']['HIGH'])) + "ms"
            for partition_xml in plan_xml.findall('Slot'):
              if partition_xml.attrib['id'] == str(0): # LO-crit partition on core 1
                partition_xml.attrib['start'] = str(0) + "ms"
                partition_xml.attrib['duration'] = slices_duration['c1']['LOW'] + "ms"
              elif partition_xml.attrib['id'] == str(1): # HI-crit partition on core 1
                partition_xml.attrib['start'] = slices_duration['c1']['LOW'] + "ms"
                partition_xml.attrib['duration'] = slices_duration['c1']['HIGH'] + "ms"
          else: # core 2
            plan_xml.attrib['majorFrame'] = str(int(slices_duration['c2']['LOW']) + int(slices_duration['c2']['HIGH'])) + "ms"
            for partition_xml in plan_xml.findall('Slot'):
              if partition_xml.attrib['id'] == str(2): # LO-crit partition on core 2
                partition_xml.attrib['start'] = str(0) + "ms"
                partition_xml.attrib['duration'] = slices_duration['c2']['LOW'] + "ms"
              elif partition_xml.attrib['id'] == str(3): # HI-crit partition on core 2:
                partition_xml.attrib['start'] = slices_duration['c2']['LOW'] + "ms"
                partition_xml.attrib['duration'] = slices_duration['c2']['HIGH'] + "ms"
        ...
        root_xm_cf.attrib['name'] = taskset_name
        tree_xm_cf.write (taskset_dir + 'xm_cf.arm.xml')
...
# It generates two XtratuM partitions, each of them concerning an Ada application on top of RTEMS (XM -> RTEMS -> Ada app)
def TSP_one_core_one_partition (experiment_id):
  common_folder = config.Ada_RTEMS_Common_Folder
        ...
        for core_XML in cores_XML:
          ...
          for task_XML in tasks_XML.findall('task'):
            ...
            if task_XML.find('criticality').text == 'HIGH':
              ...
              workload = microseconds_to_kilowhetstone_for_ravenscar_runtime( to_microseconds_for_Ada (float(task_XML.find('CHI').text)), 'HI')
            else:
              current_task += 'Low_Crit ('
              workload = microseconds_to_kilowhetstone_for_ravenscar_runtime( to_microseconds_for_Ada (float(task_XML.find('CLO').text)), 'LO')
                ...
                if task_XML.find('criticality').text == 'HIGH' and ((core_XML.tag == 'core1' and taskset.find('migonc2').text == 'TRUE') or (core_XML.tag == 'core2' and taskset.find('migonc1').text == 'TRUE')):
                  ...
                  for i in range (0, number_of_JR-1):
                    ...
                    if has_to_exceed == 1:
                      ...
                      values_for_job_release.append (int((workload * random.uniform(criticality_factor * config.TASK_MIN_REAL_UTILIZATION, criticality_factor * config.TASK_MAX_REAL_UTILIZATION))) + 1)
                    else:
                      values_for_job_release.append (int((workload * random.uniform(config.TASK_MIN_REAL_UTILIZATION, config.TASK_MAX_REAL_UTILIZATION))) + 1)
                else:
                  workload = microseconds_to_kilowhetstone_for_ravenscar_runtime( to_microseconds_for_Ada (float(task_XML.find('CLO').text)), 'LO')
...
def save_taskset_as_XML (c1_steady, c2_steady, c1_with_mig, c2_with_mig, approach, experiment_id, taskset_U, criticality_factor, hi_crit_proportion, util_on_c1, util_on_c2, taskset_id):
  c1_steady = sorted (c1_steady, key = lambda i: i['P']['c1'], reverse=True)
  #for t in c1_steady:
    #print(str(t['P']['c1']) + ": " + str(t['Ri(LO)']))
    #print(str(t['P']['c1']) + " HP:")
    # interference = t['Ri(LO)'] - t['C(LO)']

    #print ("Interference_LO", t['Interference_LO']) 
    #print ("Interference + overhead: " + str (t['Ri(LO)']) + " - " + str (t['C(LO)']) + " = " + str (t['Ri(LO)'] - t['C(LO)']))
    #print ("Overhead: " + str (t['Ri(LO)'] - t['C(LO)'] - t['Interference_LO']))

    # print("higher than " + str(t['P']['c1']) + " are:")
    # Hp = rta.findLp (t['P']['c1'], c1_steady, 'c1')
    # for ht in Hp:
    #  print (str(ht['P']['c1']))
  #print("\nmig c1\n" + str(c1_with_mig))
  #print("\nsteady c2\n" + str(c2_steady) + "\nmig c2\n" + str(c2_with_mig))
  ...
  if approach == "nomigration":
    timeslice_LO_partition_c1_XML = ET.SubElement(taskset_selector_XML, 'timesliceLOpartitionc1')
    timeslice_HI_partition_c1_XML = ET.SubElement(taskset_selector_XML, 'timesliceHIpartitionc1')
    timeslice_LO_partition_c2_XML = ET.SubElement(taskset_selector_XML, 'timesliceLOpartitionc2')
    timeslice_HI_partition_c2_XML = ET.SubElement(taskset_selector_XML, 'timesliceHIpartitionc2')

    timeslice_LO_partition_c1_XML.text = str(config.slices_duration['c1']['LOW'])
    timeslice_HI_partition_c1_XML.text = str(config.slices_duration['c1']['HIGH'])
    timeslice_LO_partition_c2_XML.text = str(config.slices_duration['c2']['LOW'])
    timeslice_HI_partition_c2_XML.text = str(config.slices_duration['c2']['HIGH'])
...
def check_size_taskset_with_mig (total, approach, experiment_id, taskset_utilization, criticality_factor, hi_crit_proportion, taskset_id):
  ...
  # we save tasksets belonging to "nomigration" approach in order to make TSP vs MCS comparison.
