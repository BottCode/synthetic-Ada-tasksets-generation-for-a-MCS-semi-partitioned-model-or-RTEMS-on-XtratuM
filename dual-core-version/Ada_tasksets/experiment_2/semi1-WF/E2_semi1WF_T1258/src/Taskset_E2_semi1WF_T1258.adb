...
package body Taskset_E2_semi1WF_T1258 is
begin
  ...
  T_2 : Low_Crit (Pri => 2, Low_Critical_Budget => 102.85141086496478, Is_Migrable => False, Workload => 0, Period => 340, CPU_Id => 1);
  T_8 : Low_Crit (Pri => 6, Low_Critical_Budget => 1.462763684842734, Is_Migrable => False, Workload => 0, Period => 10, CPU_Id => 1);
  T_7 : Low_Crit (Pri => 3, Low_Critical_Budget => 8.354242567445233, Is_Migrable => False, Workload => 0, Period => 70, CPU_Id => 1);
  T_3 : Low_Crit (Pri => 1, Low_Critical_Budget => 0.22079085862474201, Is_Migrable => False, Workload => 0, Period => 380, CPU_Id => 1);
  ...
  T_0 : Low_Crit (Pri => 4, Low_Critical_Budget => 3.0057643392435396, Is_Migrable => True, Workload => 0, Period => 90, CPU_Id => 2);

end E2_semi1WF_T1258;
